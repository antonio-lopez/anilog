...
const Trending = ({ trendingAllData, pageData }) => {
  return (
    <div className='layout'>
      <AllSectionTitle title='Trending Now' />
      <div className='grid grid-cols-1 gap-5 md:grid-cols-2'>
        {trendingAllData.map((manga) => (
          <MangaViewAllCard
            id={manga.id}
            genres={manga.genres}
            key={manga.id}
      ...
      <Pagination pageData={pageData} type='manga' category='trending' />
    </div>
  );
};
...
export async function getServerSideProps(context) {
  ...
  const { data } = await client.query({
    query: gql`
      {
        trendingAll: Page(perPage: 10, page: ${page}) {
          pageInfo {
            total
            perPage
            currentPage
  ...
  return {
    props: {
      trendingAllData: data.trendingAll.media,
      pageData: data.trendingAll.pageInfo,
    },
  };
}
